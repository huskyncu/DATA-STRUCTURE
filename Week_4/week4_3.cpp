...
int main()
{
    ...
    for(int i=0;i<spar_1.size();i++)
        cout<<spar_1[i].row<<" "<< spar_1[i].col<<" "<<spar_1[i].value<<endl;
    cout<<endl;
    for(int i=0;i<spar_2.size();i++)
        cout<<spar_2[i].row<<" "<<spar_2[i].col<<" "<<spar_2[i].value<<endl;
    cout<<endl;
    for(int i=0;i<spar_1.size();i++)
    {
        for(int j=0;i<spar_2.size();j++)
        {
            if(spar_1[i].col == spar_2[j].row)
            {
                cout << spar_1[i].col <<" "<< spar_2[j].row<<endl;
                if(mul[spar_1[i].row][spar_2[j].col] == 0)
                    counts++;
                mul[spar_1[i].row][spar_2[j].col] += (spar_1[i].value*spar_2[j].value);
                cout<<mul[spar_1[i].row][spar_2[j].col]<<endl;
            }
        }
    }
